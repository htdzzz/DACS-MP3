...
const List = ({ songData, isHideAlbum, isHideNode, order }) => {
    ...
    return (
        ...
        >
            <div className={`flex items-center gap-3 ${isHideAlbum ? 'flex-3' : 'flex-1'}`}>
                {order && <div
                    className={`${order === 1
                        ? 'text-shadow-no1'
                        : order === 2
                            ...
                            : order === 3
                                ? 'text-shadow-no3'
                                : 'text-shadow-rest'} text-main-300 text-[32px] flex items-center justify-center flex-none w-[10%]`}
                >
                    {order}
                </div>}
